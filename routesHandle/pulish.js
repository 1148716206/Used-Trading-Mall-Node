...
const pulishGoods = (req, res) => {
    const sql = `UPDATE  goods_info SET goods_name='${req.body.goods_name}',username='${req.body.username}', quality=${req.body.quality},goods_number=${req.body.goods_number},goods_desc= '${req.body.goods_desc}', old_price= ${req.body.old_price}, new_price= ${req.body.new_price},create_time='${req.body.create_time}' WHERE goods_id=${req.body.goods_id}`
    console.log(sql)
    db(sql, null, result => {
        if (result) {
          return res.send({
...
module.exports = {pulishGoods,pulishGoodsImg}